"""Train baseline PRA-prediction models off the Player_Stats table in nba_betting.db.

Mirrors predict_ast.py / predict_reb.py's architecture exactly, retargeted for
the combo prop: PRA = Points + Rebounds + Assists, a single Unified Regressor
target rather than summing three separate PTS/AST/REB projections.

Data flow
---------
1. sqlite3 -> read PLAYER_ID, GAME_DATE, PRA, USG_PCT, and the feature columns
   straight out of the `Player_Stats` table. PRA_AVG_5/PRA_AVG_10, MIN_AVG_5/
   MIN_AVG_10, REST_DAYS, and OPP_DEF_RATING are already pre-computed columns
   (player_stats.py rolled them with the same `.shift(1)` lag we use below for
   usage; PRA_AVG_5/10 were backfilled into the existing table by add_pra.py
   after player_stats.py was updated to roll PRA going forward). USG_PCT is
   the one raw column in the mix -- add_usage.py backfills it per game, but
   doesn't roll it.
...
5. scikit-learn -> fit LinearRegression and SGDRegressor on the training rows,
   predict on the held-out test rows, and score both with MAE and R^2. This
   MAE also becomes STD_DEV_PRA in daily_edge_runner.py's EV model.

Usage:
    uv run python -m nbabettingmodel.services.predict_pra
"""
...
#
# No single OPP_*_PCT column (like AST's OPP_AST_PCT or REB's OPP_REB_PCT)
# maps cleanly onto a blended points+rebounds+assists target, so opponent
# context here is scoped to OPP_DEF_RATING -- the same overall-quality signal
# every other prop model already shares -- rather than picking one component
# stat's concession rate somewhat arbitrarily.
FEATURE_COLS = [
    # Combo-prop form: trailing 5/10-game PRA averages.
    "PRA_AVG_5", "PRA_AVG_10",
    # Playing time: trailing 5/10-game minutes averages. More minutes means more
    # opportunity to accumulate points, rebounds, *and* assists at once.
    "MIN_AVG_5", "MIN_AVG_10",
    # Situational / opponent context.
    "REST_DAYS", "OPP_DEF_RATING",
]
