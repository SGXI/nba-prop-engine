"""Train baseline AST-prediction models off the Player_Stats table in nba_betting.db.

Mirrors predict_pts.py's architecture exactly, retargeted for playmaking:

Data flow
---------
1. sqlite3 -> read PLAYER_ID, GAME_DATE, AST, USG_PCT, and the feature columns
   straight out of the `Player_Stats` table. AST_AVG_5/AST_AVG_10, MIN_AVG_5/
   MIN_AVG_10, REST_DAYS, OPP_DEF_RATING, and OPP_AST_PCT are already
...
Usage:
    uv run python -m nbabettingmodel.services.predict_ast
"""
...
FEATURE_COLS = [
    # Playmaking form: trailing 5/10-game assist averages.
    "AST_AVG_5", "AST_AVG_10",
    # Playing time: trailing 5/10-game minutes averages. More minutes means more
    # possessions to accumulate assists in, almost by construction.
    "MIN_AVG_5", "MIN_AVG_10",
    # Situational / opponent context. OPP_AST_PCT -- the opponent's own share of
    # made baskets that came off an assist -- proxies for how loosely/tightly
    # that defense tends to let ball movement happen against them.
    "REST_DAYS", "OPP_DEF_RATING", "OPP_AST_PCT",
]
